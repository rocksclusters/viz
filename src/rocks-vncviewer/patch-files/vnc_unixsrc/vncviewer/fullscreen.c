...
FullScreenOn()
{
...
#ifdef ROCKS 
  int tempX, tempY;
  /* Read the initial offset into the desktop and then set the viewport
     to this */

  /* initialize the initial offset into the parent desktop */
  tempX = appData.offsetX; tempY=appData.offsetY;
  fprintf(stderr,"Offset viewport Coordinates are %d, %d \n", 
		tempX, tempY);
  fprintf(stderr,"dpy Width and Height are  %d, %d \n", 
		dpyWidth, dpyHeight);
  fprintf(stderr,"desktop Width and Height are  %d, %d \n", 
	si.framebufferWidth, si.framebufferHeight);
  /* Range limit inputs */

  if (tempX > si.framebufferWidth - dpyWidth) 
	desktopX = si.framebufferWidth - dpyWidth;
  if (desktopX < 0)
	desktopX = 0;

  if (tempY > si.framebufferHeight - dpyHeight)
	tempY = si.framebufferHeight - dpyHeight;
  if (tempY < 0)
	tempY = 0;
  ...
  XawViewportSetCoordinates(viewport, tempX, tempY);
